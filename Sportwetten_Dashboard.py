# wetten_dashboard.py
# ============================================================
# SPORTWETTEN-DASHBOARD – MIT OPTIMIERTER EINGABE
# ============================================================
...
class BettingCalculator:
    ...
    def spread_sigma(self) -> Dict[str, float]:
        total = self.expected_goals.home + self.expected_goals.away
        return {
            "1 Sigma": total * 1.3413,
            "-1 Sigma": total - 0.3413,
            "2 Sigma": total * 1.4772,
            "-2 Sigma": total - 0.4772
        }

    def spread_probabilities(self) -> Dict[str, float]:
        total = self.expected_goals.home + self.expected_goals.away
        return {
            "Unter 2,5": poisson_cumulative_under(2.5, total),
            "Über 1,5": poisson_cumulative_over(1.5, total)
        }
...
def render_sidebar():
    ...
    **Tabs:**
    ...
    """)
    st.sidebar.markdown("---")
    st.sidebar.caption("📅 v2.5 – Optimierte Eingabe")
...
def input_section() -> Tuple[TeamExpectedGoals, FullOdds]:
    ...
    # Zeile 1: Erwartete Tore + 1X2
    col_a, col_b, col_c, col_d, col_e = st.columns([1, 1, 1.5, 1.5, 1.5])
    with col_a:
        st.caption("Erwartete Tore (λ)")
        lam_h = st.number_input("Heim", min_value=0.0, value=1.75, step=0.05, format="%.2f", key="lam_h")
    with col_b:
        st.caption("")
        lam_a = st.number_input("Auswärts", min_value=0.0, value=1.5, step=0.05, format="%.2f", key="lam_a")
    with col_c:
        st.caption("1X2 – Heimsieg")
        home_win = st.number_input("Heimsieg", min_value=1.01, value=1.5, step=0.01, format="%.2f", key="hw")
    with col_d:
        st.caption("Unentschieden")
        draw = st.number_input("Unentschieden", min_value=1.01, value=4.5, step=0.01, format="%.2f", key="dr")
    with col_e:
        st.caption("Auswärtssieg")
        away_win = st.number_input("Auswärtssieg", min_value=1.01, value=5.5, step=0.01, format="%.2f", key="aw")

    expected = TeamExpectedGoals(home=lam_h, away=lam_a)
    match_odds = MatchOdds(home_win, draw, away_win)

    # Zeile 2: Über/Unter (Heim)
...
def tab_spread(calc: BettingCalculator):
    ...
    total = calc.expected_goals.home + calc.expected_goals.away
    st.metric("Kombinierte durchschnittliche Tore", f"{total:.2f}")
    ...
    fig.add_trace(go.Scatter(
        x=["-2σ", "-1σ", "μ", "+1σ", "+2σ"],
        y=[sigma["-2 Sigma"], sigma["-1 Sigma"], total, sigma["1 Sigma"], sigma["2 Sigma"]],
        mode="lines+markers",
        name="Sigma-Band",
        line=dict(color="#D4A853", width=2),
        marker=dict(size=10, color="#D4A853")
    ))
    fig.add_hline(y=total, line_dash="dash", line_color="gray", annotation_text="μ")
    fig.update_layout(
...
def tab_best_bets(calc: BettingCalculator):
    ...
    scenarios = [
        ...
        {"Fall": "Nur Ergebnis 1", "Gewinn": gain1, "Verlust": loss2},
        {"Fall": "Nur Ergebnis 2", "Gewinn": gain2, "Verlust": loss1},
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(x=df_plot["Fall"], y=df_plot["Gewinn"], name="Gewinn", marker_color="#4CAF50"))
    fig.add_trace(go.Bar(x=df_plot["Fall"], y=df_plot["Verlust"], name="Verlust", marker_color="#F44336"))
    fig.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font=dict(color="white"),
                      barmode="group", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="center", x=0.5))
    st.plotly_chart(fig, use_container_width=True)
