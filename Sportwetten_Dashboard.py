...
# MIT GUV-RECHNUNG (ALLE SZENARIEN) UND VARIABLEM EINSATZ
...
def render_sidebar():
    ...
    **Funktionen:**
    ...
    - **Alle Szenarien**: Nur eine Wette gewinnt, alle gewinnen, alle verlieren
    """)
    st.sidebar.markdown("---")
    st.sidebar.caption("📅 Wetten Dashboard v2.1")
...
def render_best_bets_and_pnl(calc: BettingCalculator):
    ...
    # --- GuV-Berechnung für alle Szenarien ---
    n = len(selected_bets)
    # Erstelle alle 2^n Kombinationen von Treffer/Niete
    # 0 = verloren, 1 = gewonnen
    scenarios = []
    for combo in product([0, 1], repeat=n):
        total_pl = 0.0
        for j, (win, bet) in enumerate(zip(combo, selected_bets)):
            if win:
                total_pl += bet["gewinn_bei_treffer"]
            else:
                total_pl += bet["verlust_bei_niete"]
        ...
        if wins == n:
            ...
            # finde die Wette, die gewinnt
            idx = combo.index(1)
            name = f"Nur '{selected_bets[idx]['label']}' gewinnt"
        else:
            # Mehrere gewinnen – wir erstellen eine Beschreibung mit den Labels
            win_labels = [selected_bets[j]['label'] for j in range(n) if combo[j] == 1]
            name = f"{', '.join(win_labels)} gewinnen"
        scenarios.append({
            "Szenario": name,
            "Gewinn/Verlust (€)": total_pl
        })

    # Erwartungswert (basierend auf den eigenen Wahrscheinlichkeiten der einzelnen Wetten)
    # Da die Wetten unabhängig sind, ist der Erwartungswert die Summe der Einzel-Erwartungswerte
    total_expected = sum(b["stake"] * b["value"] for b in selected_bets)
    scenarios.append({"Szenario": "Erwartung (Ø)", "Gewinn/Verlust (€)": total_expected})

    # In DataFrame umwandeln und nach Gewinn/Verlust sortieren (optional)
    df_scenarios = pd.DataFrame(scenarios)
    df_scenarios = df_scenarios.sort_values("Gewinn/Verlust (€)", ascending=False)

    # --- Zusammenfassung mit Metriken ---
    total_stake = sum(b["stake"] for b in selected_bets)
    max_gain = df_scenarios[df_scenarios["Szenario"] != "Erwartung (Ø)"]["Gewinn/Verlust (€)"].max()
    max_loss = df_scenarios[df_scenarios["Szenario"] != "Erwartung (Ø)"]["Gewinn/Verlust (€)"].min()
    ...
    with col2:
        st.metric("Erwarteter Gewinn", f"{total_expected:+.2f} €")
    with col3:
        st.metric("Max. Gewinn (bestes Szenario)", f"{max_gain:+.2f} €")
    with col4:
        st.metric("Max. Verlust (schlechtestes Szenario)", f"{max_loss:+.2f} €")

    # --- Tabelle mit allen Szenarien ---
    st.markdown("#### Alle möglichen Szenarien (Ergebnisse)")
    # Für bessere Lesbarkeit: Zeige nur die relevanten Szenarien, d.h. alle, aber wir können gruppieren.
    # Wir zeigen die Tabelle mit allen Szenarien.
    st.dataframe(df_scenarios.style.format({"Gewinn/Verlust (€)": "{:+.2f}"}), use_container_width=True, hide_index=True)

    # --- Zusätzlich: Tabelle mit den einzelnen Wetten und deren GuV-Beiträgen ---
    st.markdown("#### Detailierte Aufstellung der einzelnen Wetten")
    table = []
    ...
    # --- Balkendiagramm der Szenarien (nur die wichtigsten) ---
    # Wir nehmen die Szenarien "Alle gewinnen", "Alle verlieren", "Erwartung" und "Nur ..." (maximal 5)
    # Zusätzlich die besten/schlechtesten Einzel-Szenarien
    important = []
    # Füge die Extremfälle hinzu
    for name in ["Alle gewinnen", "Alle verlieren", "Erwartung (Ø)"]:
        row = df_scenarios[df_scenarios["Szenario"] == name]
        if not row.empty:
            important.append(row.iloc[0])
    # Füge die Szenarien "Nur X gewinnt" hinzu (maximal 5)
    only_wins = df_scenarios[df_scenarios["Szenario"].str.startswith("Nur '")]
    if not only_wins.empty:
        # Nimm die mit dem höchsten und niedrigsten Gewinn (um Spannweite zu zeigen)
        top_only = only_wins.nlargest(3, "Gewinn/Verlust (€)")
        bottom_only = only_wins.nsmallest(3, "Gewinn/Verlust (€)")
        for _, row in top_only.iterrows():
            if row["Szenario"] not in [s["Szenario"] for s in important]:
                important.append(row)
        for _, row in bottom_only.iterrows():
            if row["Szenario"] not in [s["Szenario"] for s in important]:
                important.append(row)
    # Falls noch Platz, füge einige andere hinzu (z.B. die mit den höchsten Wahrscheinlichkeiten? – aber wir haben keine Wahrscheinlichkeiten für Kombis, daher lassen wir es so)

    # Erstelle DataFrame für Diagramm
    df_plot = pd.DataFrame(important)
    # Sortiere nach Gewinn/Verlust für sinnvolle Darstellung
    df_plot = df_plot.sort_values("Gewinn/Verlust (€)", ascending=True)

    fig = px.bar(df_plot, x="Szenario", y="Gewinn/Verlust (€)", color="Szenario",
